...
public isolated client class librarysytemClient {
    *grpc:AbstractClientEndpoint;
    ...
    public isolated function init(string url, *grpc:ClientConfiguration config) returns grpc:Error? {
        ...
        check self.grpcClient.initStub(self, USER_DESC);
    }

    isolated remote function createUser(Course|ContextUser req) returns Responds|grpc:Error {
    ...
    isolated remote function createUserContext(Course|ContextUser req) returns ContextResponds|grpc:Error {
        map<string|string[]> headers = {};
        Course message;
        if req is ContextUser {
            message = req.content;
    ...
    isolated remote function updateBook(Course|ContextBook req) returns Responds|grpc:Error {
        map<string|string[]> headers = {};
        Course message;
    ...
    isolated remote function updateBookContext(Course|ContextBook req) returns ContextResponds|grpc:Error {
        map<string|string[]> headers = {};
        Course message;
        if req is ContextBook {
            message = req.content;
    ...
    isolated remote function removeBook(string|wrappers:ContextString req) returns Responds|grpc:Error {
        map<string|string[]> headers = {};
        string message;
        ...
        var payload = check self.grpcClient->executeSimpleRPC("librarysystem.librarysystem/removeBook", message, headers);
        [anydata, map<string|string[]>] [result, _] = payload;
        return <Responds>result;
    }

    isolated remote function removeBookContext(string|wrappers:ContextString req) returns ContextResponds|grpc:Error {
        map<string|string[]> headers = {};
        string message;
        if req is wrappers:ContextString {
            message = req.content;
...
public client class librarysystemRespondsCaller {
    private grpc:Caller caller;
...
@protobuf:Descriptor {value: USER_DESC}
